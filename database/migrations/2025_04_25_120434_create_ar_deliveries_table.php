<?php

use Illuminate\Database\Migrations\Migration;
use Illuminate\Database\Schema\Blueprint;
use Illuminate\Support\Facades\Schema;

return new class extends Migration
{
    /**
     * Run the migrations.
     *
     * @return void
     */
    public function up()
    {
        Schema::create('ar_deliveries', function (Blueprint $table) {
            $table->bigIncrements('id');
            $table->string('parent_slug')->nullable();
            $table->integer('parent_id')->nullable();
            $table->bigInteger('sort');
            $table->integer('report_id');
            $table->timestamp('created_at')->useCurrent();
            $table->timestamp('updated_at')->useCurrentOnUpdate()->useCurrent();
            $table->string('image')->nullable();
            $table->string('title')->nullable();
            $table->double('base_price')->nullable();
            $table->double('price')->nullable();
            $table->string('hurry_price')->nullable();
            $table->string('image_waiting')->nullable();
            $table->string('title_waiting')->nullable();
            $table->string('description_waiting')->nullable();
            $table->double('back_price')->nullable();
            $table->string('token')->nullable();
            $table->integer('type')->nullable();
            $table->double('decrease_percent')->nullable();
            $table->string('economic_icon')->nullable();
            $table->double('time_price')->nullable();
            $table->integer('service')->nullable();
            $table->tinyInteger('show')->nullable();
            $table->tinyInteger('have_economic')->nullable();
        });
    }

    /**
     * Reverse the migrations.
     *
     * @return void
     */
    public function down()
    {
        Schema::dropIfExists('ar_deliveries');
    }
};
