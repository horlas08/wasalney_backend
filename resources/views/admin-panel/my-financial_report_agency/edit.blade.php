...
@section('content')
    ...
    <form action="{{route('record.update',['slug'=>'financial_report_agency','id'=>$financial_report_agency->id])}}"
                ...
                <div class="card" id="form-box">
                    ...
                    <div id="sticky-wrapper" class="sticky-wrapper" >
                        <div class="card-header sticky-element bg-label-secondary d-flex justify-content-sm-between align-items-sm-center flex-column flex-sm-row" style="">
                            <h5 class="card-title mb-sm-0 me-2">{{ __('Edit') }} {{__("گزارشات مالی آژانس")}}</h5>
                            <div class="action-btns ">
                                <a href="{{route('record.index',['slug'=>'financial_report_agency'])}}" class="btn btn-outline-primary me-3">
                            ...
                            <div class="row mb-5">
                                <div class="col-lg-11 mx-auto">
                                    <div class="row g-3">
                                        ...
                                        <div class="col-md-12">
                                        <label class="form-label" for="order">{{__("سفر")}}</label>
                                        ...
                                        <option {{$financial_report_agency->order==$orders->id?"selected":""}} value="{{$orders->id}}">{{"$orders->code"}}</option>
                                        @endforeach
                                        </select>
                                        </div>
                                        <div class="col-md-12">
                                        <label class="form-label" for="price">{{__("مبلغ")}}</label>
                                        <input id="price" type="number" name="price" value="{{$financial_report_agency->price}}" class="form-control"  />
                                        </div>
                                        <div class="col-md-12">
                                        <label class="form-label" for="admin">{{__("ادمین")}}</label>
                                        <select id="admin" name="admin" class="select2 form-select  form-select-lg"  data-width="100%">
                                        <option value=" ">{{__('not selected')}}</option>
                                        @foreach(\App\Models\MyAgency_Admin::orderBy('sort','DESC')->get() as $agency_admin)
                                        <option {{$financial_report_agency->admin==$agency_admin->id?"selected":""}} value="{{$agency_admin->id}}">{{"$agency_admin->title"}}</option>
                                        @endforeach
                                        </select>
                                        </div>
...
@section('scripts')
    ...
    <script>
        ...
        @php($lastCategory=\App\Models\Category::where("slug",'financial_report_agency')->first())
