...
@section('content')
    ...
    <form action="{{route('record.update',['slug'=>'worker_price','id'=>$worker_price->id])}}"
                ...
                <div class="card" id="form-box">
                    ...
                    <div id="sticky-wrapper" class="sticky-wrapper" >
                        <div class="card-header sticky-element bg-label-secondary d-flex justify-content-sm-between align-items-sm-center flex-column flex-sm-row" style="">
                            <h5 class="card-title mb-sm-0 me-2">{{ __('Edit') }} {{__("تعیین قیمت هر کارگر باربری")}}</h5>
                            <div class="action-btns ">
                                <a href="{{route('record.index',['slug'=>'worker_price','parentSlug'=>$parentSlug,'parentId'=>$parentId])}}" class="btn btn-outline-primary me-3">
                            ...
                            <div class="row mb-5">
                                <div class="col-lg-11 mx-auto">
                                    <div class="row g-3">
                                        ...
                                        <input id="name" type="text" name="name" value="{{$worker_price->name}}" class="form-control"  />
                                        </div>
                                        <div class="col-md-12">
                                        <label class="form-label" for="description">{{__("توضیحات")}}</label>
                                        <input id="description" type="text" name="description" value="{{$worker_price->description}}" class="form-control"  />
                                        </div>
                                        <div class="col-md-12">
                                        <label class="form-label" for="price">{{__("قیمت")}}</label>
                                        <input id="price" type="number" name="price" value="{{$worker_price->price}}" class="form-control"  />
                                        </div>
