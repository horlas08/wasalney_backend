...
@section('content')
    ...
    <form action="{{route('record.update',['slug'=>'discount_order','id'=>$discount_order->id])}}"
                ...
                <div class="card" id="form-box">
                    ...
                    <div id="sticky-wrapper" class="sticky-wrapper" >
                        <div class="card-header sticky-element bg-label-secondary d-flex justify-content-sm-between align-items-sm-center flex-column flex-sm-row" style="">
                            <h5 class="card-title mb-sm-0 me-2">{{ __('Edit') }} {{__("سفارشات تخفیفی")}}</h5>
                            <div class="action-btns ">
                                <a href="{{route('record.index',['slug'=>'discount_order','parentSlug'=>$parentSlug,'parentId'=>$parentId])}}" class="btn btn-outline-primary me-3">
                            ...
                            <div class="row mb-5">
                                <div class="col-lg-11 mx-auto">
                                    <div class="row g-3">
                                        ...
                                        <option {{$discount_order->user==$users->id?"selected":""}} value="{{$users->id}}">{{"($users->mobile) $users->name"}}</option>
                                        @endforeach
                                        </select>
                                        </div>
                                        <div class="col-md-12">
                                        ...
                                        <option {{$discount_order->order==$orders->id?"selected":""}} value="{{$orders->id}}">{{"$orders->code"}}</option>
                                        @endforeach
                                        </select>
                                        </div>
