...
@section('content')
    ...
    <form action="{{route('record.update',['slug'=>'wallet_agency','id'=>$wallet_agency->id])}}"
                ...
                <div class="card" id="form-box">
                    ...
                    <div id="sticky-wrapper" class="sticky-wrapper" >
                        <div class="card-header sticky-element bg-label-secondary d-flex justify-content-sm-between align-items-sm-center flex-column flex-sm-row" style="">
                            <h5 class="card-title mb-sm-0 me-2">{{ __('Edit') }} {{__("کیف پول آژانس")}}</h5>
                            <div class="action-btns ">
                                <a href="{{route('record.index',['slug'=>'wallet_agency'])}}" class="btn btn-outline-primary me-3">
                            ...
                            <div class="row mb-5">
                                <div class="col-lg-11 mx-auto">
                                    <div class="row g-3">
                                        ...
                                        <input id="price" type="number" name="price" value="{{$wallet_agency->price}}" class="form-control"  />
                                        </div>
                                        <div class="col-md-12">
                                        <label class="form-label" for="type">{{__("نوع عملیات")}}</label>
                                        ...
                                        <option {{$wallet_agency->type==$transaction_types->id?"selected":""}} value="{{$transaction_types->id}}">{{"$transaction_types->title"}}</option>
                                        @endforeach
                                        </select>
                                        </div>
