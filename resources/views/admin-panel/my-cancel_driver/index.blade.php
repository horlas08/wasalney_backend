...
@section('content')
    ...
    @php($parent=$parentSlug!=null?db($parentSlug)->find($parentId):null)
        ...
        <h4 class="py-3 breadcrumb-wrapper  col-4 ">
            ...
            @endwhile
            {{__("الطلبات الملغية من قبل السائق")}}
            </h4>
        ...
        <div class="d-flex row col-4 flex-nowrap justify-content-end">
            ...
            <div class="btn-group" style="width: auto;">
                ...
                <ul class="dropdown-menu" style="position: absolute; inset: 0px auto auto 0px; margin: 0px; transform: translate(0px, 38px);" data-popper-placement="bottom-start">
                    @if(\App\Models\Admin::checkAccess('record','store',"cancel_driver"))
                        <li>
                            <a  href="{{route('record.storeform',['slug'=>'cancel_driver','parentSlug'=>$parentSlug,'parentId'=>$parentId])}}"  type="button" class="dropdown-item btn ">
                    ...
                    @endif
                    <li>
                        <a href="{{route('record.export',['slug'=>'cancel_driver','parentSlug'=>$parentSlug,'parentId'=>$parentId])}}"  type="button" class="dropdown-item btn ">
                            <i class="fa-solid fa-file-excel me-1"></i>
                            {{ __('Excel Export') }}
                        </a>
                    </li>
                    <li>
                        <a href="{{route('record.pdf',['slug'=>'cancel_driver','parentSlug'=>$parentSlug,'parentId'=>$parentId])}}"  type="button" class="dropdown-item btn ">
        ...
        <div class="row">
            <div class="card" id="table-box" >
                ...
                <div class="card-datatable table-responsive">
                    <table id="ajax_table" class="table table-bordered " sortLink="{{route('record.sort',['slug'=>'cancel_driver'])}}">
                        <thead>
                        <tr>
                            <th>#</th>
...
@section('scripts')
    ...
    <script>
        ...
        $('#ajax_table').DataTable({
            "ajax":"{{route('record.pagination',['slug'=>'cancel_driver','parentSlug'=>$parentSlug,'parentId'=>$parentId])}}",
            "columns":[
            ...
            "processing": true,
            "serverSide": true,
            @if($tableInfo!=null && $tableInfo->slug=="cancel_driver" && $tableInfo->parentSlug==$parentSlug && $tableInfo->parentId==$parentId)
        ...
        @php($lastCategory=isset($lastCategory)?$lastCategory:(\App\Models\Category::where("slug",'cancel_driver')->first()))
