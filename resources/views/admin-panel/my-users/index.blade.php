        ...
        <div class="d-flex row col-4 flex-nowrap justify-content-end">
            ...
            <div class="btn-group" style="width: auto;">
                ...
                <ul class="dropdown-menu" style="position: absolute; inset: 0px auto auto 0px; margin: 0px; transform: translate(0px, 38px);" data-popper-placement="bottom-start">
                    @if(\App\Models\Admin::checkAccess('record','store',"users"))
                        <li>
                            <a  href="{{route('record.storeform',['slug'=>'users'])}}"  type="button" class="dropdown-item btn ">
                    ...
                    @endif
                    <li>
                        <a href="{{route('record.export',['slug'=>'users'])}}"  type="button" class="dropdown-item btn ">
                            <i class="fa-solid fa-file-excel me-1"></i>
                            {{ __('Excel Export') }}
                        </a>
                    </li>
                    <li>
                        <a href="{{route('record.pdf',['slug'=>'users'])}}"  type="button" class="dropdown-item btn ">
    ...
    <div class=" widgets" >
        <div class="row">
            <div class="card" id="table-box" >
                {{--start--}}
                <form method="get" action="{{route('record.index',['slug'=>'users'])}}"
                      class="w-100 d-flex justify-content-center align-items-end">

                    <div class="m-2">
                        <label class="form-label" for="from">{{__('کیف پول از')}}</label>
                        <input name="from" type="number" id="from" class="form-control" value="{{app('request')->input('from')}}">
                    </div>
                    <div class="m-2">
                        <label class="form-label" for="to">{{__('کیف پول تا')}}</label>
                        <input name="to" type="number" id="to" class="form-control" value="{{app('request')->input('to')}}">
                    </div>
                ...
                <div class="card-datatable table-responsive">
                    <table id="ajax_table" class="table table-bordered " sortLink="{{route('record.sort',['slug'=>'users'])}}">
                        <thead>
                        <tr>
                            <th>#</th>
...
@section('scripts')
    ...
    <script>
        ...
        $('#ajax_table').DataTable({
            "ajax":"{{route('record.pagination',['slug'=>'users'])}}",
            "columns":[
            ...
            "processing": true,
            "serverSide": true,
            @if($tableInfo!=null && $tableInfo->slug=="users" && $tableInfo->parentSlug==null && $tableInfo->parentId==null)
