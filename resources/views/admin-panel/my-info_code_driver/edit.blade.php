...
@section('content')
    ...
    <form action="{{route('record.update',['slug'=>'info_code_driver','id'=>$info_code_driver->id])}}"
                ...
                <div class="card" id="form-box">
                    ...
                    <div id="sticky-wrapper" class="sticky-wrapper" >
                        <div class="card-header sticky-element bg-label-secondary d-flex justify-content-sm-between align-items-sm-center flex-column flex-sm-row" style="">
                            <h5 class="card-title mb-sm-0 me-2">{{ __('Edit') }} {{__("معلومات رمز الرصید السائق")}}</h5>
                            <div class="action-btns ">
                                <a href="{{route('record.index',['slug'=>'info_code_driver','parentSlug'=>$parentSlug,'parentId'=>$parentId])}}" class="btn btn-outline-primary me-3">
                            ...
                            <div class="row mb-5">
                                <div class="col-lg-11 mx-auto">
                                    <div class="row g-3">
                                        ...
                                        <div class="col-md-12">
                                        <label class="form-label" for="code">{{__("کد")}}</label>
                                        <input id="code" type="text" name="code" value="{{$info_code_driver->code}}" class="form-control"  />
                                        </div>
                                        <div class="col-md-12">
                                        <label class="form-label" for="price">{{__("القيمة")}}</label>
                                        <input id="price" type="number" name="price" value="{{$info_code_driver->price}}" class="form-control"  />
                                        </div>
                                        <div class="col-md-12">
                                        <input type="hidden" name="used" value="{{ $info_code_driver->used }}">
                                        </div>
                                        <div class="col-md-12">
                                        <label class="form-label" for="driver">{{__("السائق")}}</label>
                                        ...
                                        <option {{$info_code_driver->driver==$drivers->id?"selected":""}} value="{{$drivers->id}}">{{"$drivers->name $drivers->mobile"}}</option>
                                        @endforeach
                                        </select>
                                        </div>
