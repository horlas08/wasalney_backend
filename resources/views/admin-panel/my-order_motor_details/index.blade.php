        ...
        <div class="d-flex row col-4 flex-nowrap justify-content-end">
            ...
            <div class="btn-group" style="width: auto;">
                ...
                <ul class="dropdown-menu" style="position: absolute; inset: 0px auto auto 0px; margin: 0px; transform: translate(0px, 38px);" data-popper-placement="bottom-start">
                    @if(\App\Models\Admin::checkAccess('record','store',"order_motor_details"))
                        <li>
                            <a  href="{{route('record.storeform',['slug'=>'order_motor_details'])}}"  type="button" class="dropdown-item btn ">
                    ...
                    @endif
                    <li>
                        <a href="{{route('record.export',['slug'=>'order_motor_details'])}}"  type="button" class="dropdown-item btn ">
                            <i class="fa-solid fa-file-excel me-1"></i>
                            {{ __('Excel Export') }}
                        </a>
                    </li>
                    <li>
                        <a href="{{route('record.pdf',['slug'=>'order_motor_details'])}}"  type="button" class="dropdown-item btn ">
    ...
    <div class=" widgets" >
        <div class="row">
            <div class="card" id="table-box" >
                {{--start--}}
                <form method="get" action="{{route('record.index',['slug'=>'order_motor_details'])}}"
                      class="w-100 d-flex justify-content-center align-items-end">

                    <div class="m-2" >
                        <label for="type" class="form-label">{{__('نوع مرسوله')}}</label>
                        <select id="type" name="type" class="select2 form-select form-select-lg">
                            <option value="0">{{__('choose')}}</option>
                            @foreach(db('type_parcel')->getRecords() as $type)
                                <option {{app('request')->input('type')==$type->id?'selected="selected"':''}} value="{{$type->id}}">{{$type->title}}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="m-2" >
                        <label for="price_from" class="form-label">{{__('حداقل ارزش مرسوله')}}</label>
                        <input  id="price_from" name="price_from"  class="form-control" value="{{app('request')->input('price_from')}}">
                    </div>
                    <div class="m-2" >
                        <label for="price_to" class="form-label">{{__('حداکثر ارزش مرسوله')}} </label>
                        <input  id="price_to" name="price_to"  class="form-control" value="{{app('request')->input('price_to')}}">
                    </div>
                ...
                <div class="card-datatable table-responsive">
                    <table id="ajax_table" class="table table-bordered " sortLink="{{route('record.sort',['slug'=>'order_motor_details'])}}">
                        <thead>
                        <tr>
                            <th>#</th>
...
@section('scripts')
    ...
    <script>
        ...
        $('#ajax_table').DataTable({
            "ajax":"{{route('record.pagination',['slug'=>'order_motor_details'])}}",
            "columns":[
            ...
            "processing": true,
            "serverSide": true,
            @if($tableInfo!=null && $tableInfo->slug=="order_motor_details" && $tableInfo->parentSlug==null && $tableInfo->parentId==null)
        ...
        @php($lastCategory=\App\Models\Category::where("slug",'order_motor_details')->first())
