...
@section('content')
    ...
    <form action="{{route('record.update',['slug'=>'commission','id'=>$commission->id])}}"
                ...
                <div class="card" id="form-box">
                    ...
                    <div id="sticky-wrapper" class="sticky-wrapper" >
                        <div class="card-header sticky-element bg-label-secondary d-flex justify-content-sm-between align-items-sm-center flex-column flex-sm-row" style="">
                            <h5 class="card-title mb-sm-0 me-2">{{ __('Edit') }} {{__("کمیسیون")}}</h5>
                            <div class="action-btns ">
                                <a href="{{route('record.index',['slug'=>'commission','parentSlug'=>$parentSlug,'parentId'=>$parentId])}}" class="btn btn-outline-primary me-3">
                            ...
                            <div class="row mb-5">
                                <div class="col-lg-11 mx-auto">
                                    <div class="row g-3">
                                        ...
                                        <div class="col-md-12">
                                        <label class="form-label" for="level">{{__("نوع سطح")}}</label>
                                        ...
                                        <option {{$commission->level==$level->id?"selected":""}} value="{{$level->id}}">{{"$level->title"}}</option>
                                        @endforeach
                                        </select>
                                        </div>
                                        <div class="col-md-12">
                                        <label class="form-label" for="percent">{{__("درصد کمیسیون")}}</label>
                                        <input id="percent" type="text" name="percent" value="{{$commission->percent}}" class="form-control"  />
                                        </div>
