        ...
        <div class="d-flex row col-4 flex-nowrap justify-content-end">
            ...
            <div class="btn-group" style="width: auto;">
                ...
                <ul class="dropdown-menu" style="position: absolute; inset: 0px auto auto 0px; margin: 0px; transform: translate(0px, 38px);" data-popper-placement="bottom-start">
                    @if(\App\Models\Admin::checkAccess('record','store',"wallet_admin"))
                        <li>
                            <a  href="{{route('record.storeform',['slug'=>'wallet_admin'])}}"  type="button" class="dropdown-item btn ">
                    ...
                    @endif
                    <li>
                        <a href="{{route('record.export',['slug'=>'wallet_admin'])}}"  type="button" class="dropdown-item btn ">
                            <i class="fa-solid fa-file-excel me-1"></i>
                            {{ __('Excel Export') }}
                        </a>
                    </li>
                    <li>
                        <a href="{{route('record.pdf',['slug'=>'wallet_admin'])}}"  type="button" class="dropdown-item btn ">
        ...
        <div class="row">
            <div class="card" id="table-box" >
                ...
                <div class="card-datatable table-responsive">
                    <table id="ajax_table" class="table table-bordered " sortLink="{{route('record.sort',['slug'=>'wallet_admin'])}}">
                        <thead>
                        <tr>
                            <th>#</th>
...
@section('scripts')
    ...
    <script>
        ...
        $('#ajax_table').DataTable({
            "ajax":"{{route('record.pagination',['slug'=>'wallet_admin'])}}",
            "columns":[
            ...
            "processing": true,
            "serverSide": true,
            @if($tableInfo!=null && $tableInfo->slug=="wallet_admin" && $tableInfo->parentSlug==null && $tableInfo->parentId==null)
        ...
        @php($lastCategory=\App\Models\Category::where("slug",'wallet_admin')->first())
