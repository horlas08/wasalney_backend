...
@section('content')
    ...
    <form action="{{route('record.update',['slug'=>'admin_message','id'=>$admin_message->id])}}"
                ...
                <div class="card" id="form-box">
                    ...
                    <div id="sticky-wrapper" class="sticky-wrapper" >
                        <div class="card-header sticky-element bg-label-secondary d-flex justify-content-sm-between align-items-sm-center flex-column flex-sm-row" style="">
                            <h5 class="card-title mb-sm-0 me-2">{{ __('Edit') }} {{__("رسالة الادمن للسائق")}}</h5>
                            <div class="action-btns ">
                                <a href="{{route('record.index',['slug'=>'admin_message'])}}" class="btn btn-outline-primary me-3">
                            ...
                            <div class="row mb-5">
                                <div class="col-lg-11 mx-auto">
                                    <div class="row g-3">
                                        ...
                                        <div class="col-md-12">
                                        <label class="form-label" for="driver">{{__("رانندگان")}}</label>
                                        <select id="driver" name="driver[]" class="select2 form-select  form-select-lg" multiple="multiple" data-width="100%">
                                        @php($values=\App\Models\MyAdmin_MessageDrivers::where('admin_message_id',$admin_message->id)->get())
                                        @foreach(\App\Models\MyDrivers::orderBy('sort','DESC')->get() as $drivers)
                                        <option {{$values->where('drivers_id',$drivers->id)->first()!=null?"selected":""}} value="{{$drivers->id}}">{{"($drivers->mobile) $drivers->name"}}</option>
                                        @endforeach
                                        </select>
                                        </div>
                                        <div class="col-md-12">
                                        <label class="form-label" for="message">{{__("الرسالة")}}</label>
                                        <input id="message" type="text" name="message" value="{{$admin_message->message}}" class="form-control"  />
                                        </div>
