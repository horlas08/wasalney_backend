...
@section('content')
    ...
    <form action="{{route('record.update',['slug'=>'drivers_send','id'=>$drivers_send->id])}}"
                ...
                <div class="card" id="form-box">
                    ...
                    <div id="sticky-wrapper" class="sticky-wrapper" >
                        <div class="card-header sticky-element bg-label-secondary d-flex justify-content-sm-between align-items-sm-center flex-column flex-sm-row" style="">
                            <h5 class="card-title mb-sm-0 me-2">{{ __('Edit') }} {{__("drivers_send")}}</h5>
                            <div class="action-btns ">
                                <a href="{{route('record.index',['slug'=>'drivers_send','parentSlug'=>$parentSlug,'parentId'=>$parentId])}}" class="btn btn-outline-primary me-3">
                            ...
                            <div class="row mb-5">
                                <div class="col-lg-11 mx-auto">
                                    <div class="row g-3">
                                        ...
                                        <option {{$drivers_send->driver==$drivers->id?"selected":""}} value="{{$drivers->id}}">{{"$drivers->name $drivers->mobile"}}</option>
                                        @endforeach
                                        </select>
                                        </div>
