...
@section('content')
    ...
    <form action="{{route('record.update',['slug'=>'info_code','id'=>$info_code->id])}}"
                ...
                <div class="card" id="form-box">
                    ...
                    <div id="sticky-wrapper" class="sticky-wrapper" >
                        <div class="card-header sticky-element bg-label-secondary d-flex justify-content-sm-between align-items-sm-center flex-column flex-sm-row" style="">
                            <h5 class="card-title mb-sm-0 me-2">{{ __('Edit') }} {{__("معلومات رمز الرصید المستخدم")}}</h5>
                            <div class="action-btns ">
                                <a href="{{route('record.index',['slug'=>'info_code','parentSlug'=>$parentSlug,'parentId'=>$parentId])}}" class="btn btn-outline-primary me-3">
                            ...
                            <div class="row mb-5">
                                <div class="col-lg-11 mx-auto">
                                    <div class="row g-3">
                                        ...
                                        <input id="code" type="text" name="code" value="{{$info_code->code}}" class="form-control"  />
                                        </div>
                                        <div class="col-md-12">
                                        <label class="form-label" for="price">{{__("ارزش")}}</label>
                                        <input id="price" type="number" name="price" value="{{$info_code->price}}" class="form-control"  />
                                        </div>
                                        <div class="col-md-12">
                                        <input type="hidden" name="used" value="{{ $info_code->used }}">
                                        </div>
                                        <div class="col-md-12">
                                        ...
                                        <option {{$info_code->user==$users->id?"selected":""}} value="{{$users->id}}">{{"($users->mobile)  $users->name"}}</option>
                                        @endforeach
                                        </select>
                                        </div>
