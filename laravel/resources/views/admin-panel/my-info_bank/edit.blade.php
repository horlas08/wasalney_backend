...
@section('content')
    ...
    <form action="{{route('record.update',['slug'=>'info_bank','id'=>$info_bank->id])}}"
                ...
                <div class="card" id="form-box">
                    ...
                    <div id="sticky-wrapper" class="sticky-wrapper" >
                        <div class="card-header sticky-element bg-label-secondary d-flex justify-content-sm-between align-items-sm-center flex-column flex-sm-row" style="">
                            <h5 class="card-title mb-sm-0 me-2">{{ __('Edit') }} {{__("اطلاعات بانکی")}}</h5>
                            <div class="action-btns ">
                                <a href="{{route('record.index',['slug'=>'info_bank','parentSlug'=>$parentSlug,'parentId'=>$parentId])}}" class="btn btn-outline-primary me-3">
                            ...
                            <div class="row mb-5">
                                <div class="col-lg-11 mx-auto">
                                    <div class="row g-3">
                                        ...
                                        <div class="col-md-12">
                                        <label class="form-label" for="cart_number">{{__("شماره کارت")}}</label>
                                        <input id="cart_number" type="text" name="cart_number" value="{{$info_bank->cart_number}}" class="form-control"  />
                                        </div>
                                        <div class="col-md-12">
                                        <label class="form-label" for="shaba">{{__("شماره شبا کارت")}}</label>
                                        <input id="shaba" type="text" name="shaba" value="{{$info_bank->shaba}}" class="form-control"  />
                                        </div>
                                        <div class="col-md-12">
                                        <label class="form-label" for="name">{{__("نام صاحب حساب")}}</label>
                                        <input id="name" type="text" name="name" value="{{$info_bank->name}}" class="form-control"  />
                                        </div>
                                        <div class="col-md-12">
                                        <label class="form-label" for="bank">{{__("نام بانک")}}</label>
                                        <select id="bank" name="bank" class="select2 form-select  form-select-lg"  data-width="100%">
                                        <option value=" ">{{__('not selected')}}</option>
                                        @foreach(\App\Models\MyBanks::orderBy('sort','DESC')->get() as $banks)
                                        <option {{$info_bank->bank==$banks->id?"selected":""}} value="{{$banks->id}}">{{"$banks->title"}}</option>
                                        @endforeach
                                        </select>
                                        </div>
...
@section('scripts')
    ...
    <script>
        ...
        @endwhile
        @php($lastCategory=isset($lastCategory)?$lastCategory:(\App\Models\Category::where("slug",'info_bank')->first()))
