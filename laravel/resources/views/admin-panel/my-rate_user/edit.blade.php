...
@section('content')
    ...
    <form action="{{route('record.update',['slug'=>'rate_user','id'=>$rate_user->id])}}"
                ...
                <div class="card" id="form-box">
                    ...
                    <div id="sticky-wrapper" class="sticky-wrapper" >
                        <div class="card-header sticky-element bg-label-secondary d-flex justify-content-sm-between align-items-sm-center flex-column flex-sm-row" style="">
                            <h5 class="card-title mb-sm-0 me-2">{{ __('Edit') }} {{__("امتیاز کاربر")}}</h5>
                            ...
                            <div class="row mb-5">
                                <div class="col-lg-11 mx-auto">
                                    <div class="row g-3">
                                        ...
                                        <option {{$rate_user->type==$transaction_types->id?"selected":""}} value="{{$transaction_types->id}}">{{"$transaction_types->title"}}</option>
                                        @endforeach
                                        </select>
                                        </div>
                                        <div class="col-md-12">
                                        <label class="form-label" for="count">{{__("تعداد")}}</label>
                                        <input id="count" type="number" name="count" value="{{$rate_user->count}}" class="form-control"  />
                                        </div>
                                        <div class="col-md-12">
                                        <label class="form-label" for="cost">{{__("ارزش")}}</label>
                                        <input id="cost" type="number" name="cost" value="{{$rate_user->cost}}" class="form-control"  />
                                        </div>
