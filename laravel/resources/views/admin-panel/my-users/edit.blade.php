...
@section('content')
    ...
    <form action="{{route('record.update',['slug'=>'users','id'=>$users->id])}}"
                ...
                <div class="card" id="form-box">
                    ...
                    <div id="sticky-wrapper" class="sticky-wrapper" >
                        <div class="card-header sticky-element bg-label-secondary d-flex justify-content-sm-between align-items-sm-center flex-column flex-sm-row" style="">
                            <h5 class="card-title mb-sm-0 me-2">{{ __('Edit') }} {{__("کاربران")}}</h5>
                            <div class="action-btns ">
                                <a href="{{route('record.index',['slug'=>'users'])}}" class="btn btn-outline-primary me-3">
                            ...
                            <div class="row mb-5">
                                <div class="col-lg-11 mx-auto">
                                    <div class="row g-3">
                                        ...
                                        <input id="email" type="text" name="email" value="{{$users->email}}" class="form-control"  />
                                        </div>
                                        <div class="col-md-12">
                                        ...
                                        <input id="address" type="text" name="address" value="{{$users->address}}" class="form-control"  />
                                        </div>
                                        <div class="col-md-12">
                                        <label class="form-label" for="mobile">{{__("موبایل")}}</label>
                                        <input id="mobile" type="text" name="mobile" value="{{$users->mobile}}" class="form-control"  />
                                        </div>
                                        <div class="col-md-12">
                                        <label class="form-label" for="verify_code">{{__("verify code")}}</label>
                                        <input id="verify_code" type="text" name="verify_code" value="{{$users->verify_code}}" class="form-control"  />
                                        </div>
                                        <div class="col-md-12">
                                        <label class="form-label" for="image">{{__("پروفایل")}}</label>
                                        <input type="file" name="image" class="form-control" >
                                        <div style="display: flex" deleteUrl="{{route('file.delete',['slug'=>'users','fieldName'=>'image','id'=>$users->id])}}">
                                        @if($users->image!=null)                                        <div style="position: relative">
                                        <a href="{{"/files/".$users->image}}" target="_blank">
                                        <img src="{{"/files/".$users->image}}" height="60" width="60" onerror="this.src='/admin-panel/images/file.png'">
                                        </a>
                                        <i id="btnDelete-image" onclick="deleteFile(this,'image')" fileName="{{last(explode('/',$users->image))}}" class="fa-solid fa-trash" style="position: absolute;top: 5px;right: 5%;color: red;cursor: pointer"></i>
                                        </div>
                                        @endif
                                        </div>
                                        </div>
                                        <div class="col-md-12">
                                        <label class="form-label" for="birth_date">{{__("تاریخ تولد")}}</label>
                                      
                                        @if(getLang()=='fa')
                                        <input data-jdp name="birth_date"  type="text" id="birth_date" class="form-control" value="{{$users->birth_date!=null?\Morilog\Jalali\Jalalian::fromCarbon(\Illuminate\Support\Carbon::createFromFormat('Y-m-d', $users->birth_date))->format('Y/m/d'):""}}">
                                        @else
                                        <input id="birth_date" type="date" name="birth_date" class="form-control" value="{{$users->birth_date->main}}"/>
                                        @endif
                                        </div>
                                        <div class="col-md-12">
                                        <label class="form-label" for="code">{{__("کد معرفی")}}</label>
                                        <input id="code" type="text" name="code" value="{{$users->code}}" class="form-control"  />
                                        </div>
                                        <div class="col-md-12">
                                        <label class="form-label" for="intro_code">{{__("کد معرف استفاده شده")}}</label>
                                        <input id="intro_code" type="text" name="intro_code" value="{{$users->intro_code}}" class="form-control"  />
                                        </div>
                                        <div class="col-md-12">
                                        <label class="form-label" for="credit">{{__("اعتبار کیف پول")}}</label>
                                        <input id="credit" type="number" name="credit" value="{{$users->credit}}" class="form-control"  />
                                        </div>
...
@section('scripts')
    ...
    <script>
        ...
        @php($lastCategory=\App\Models\Category::where("slug",'users')->first())
