...
@section('content')
    ...
    <form action="{{route('record.update',['slug'=>'repetitive_routes','id'=>$repetitive_routes->id])}}"
                ...
                <div class="card" id="form-box">
                    ...
                    <div id="sticky-wrapper" class="sticky-wrapper" >
                        <div class="card-header sticky-element bg-label-secondary d-flex justify-content-sm-between align-items-sm-center flex-column flex-sm-row" style="">
                            <h5 class="card-title mb-sm-0 me-2">{{ __('Edit') }} {{__("مسیرهای پر تکرار")}}</h5>
                            <div class="action-btns ">
                                <a href="{{route('record.index',['slug'=>'repetitive_routes'])}}" class="btn btn-outline-primary me-3">
                            ...
                            <div class="row mb-5">
                                <div class="col-lg-11 mx-auto">
                                    <div class="row g-3">
                                        ...
                                        <option {{$repetitive_routes->origin==$favorite_place->id?"selected":""}} value="{{$favorite_place->id}}">{{"$favorite_place->title"}}</option>
                                        @endforeach
                                        </select>
                                        </div>
                                        <div class="col-md-12">
                                        ...
                                        <option value=" ">{{__('not selected')}}</option>
                                        @foreach(\App\Models\MyFavorite_Place::orderBy('sort','DESC')->get() as $favorite_place)
                                        <option {{$repetitive_routes->destination==$favorite_place->id?"selected":""}} value="{{$favorite_place->id}}">{{"$favorite_place->title"}}</option>
                                        @endforeach
                                        </select>
                                        </div>
                                        <div class="col-md-12">
                                        <label class="form-label" for="count">{{__("تعداد استفاده شده")}}</label>
                                        <input id="count" type="number" name="count" value="{{$repetitive_routes->count}}" class="form-control"  />
                                        </div>
                                        <div class="col-md-12">
                                        <label class="form-label" for="delivery">{{__("نوع سرویس")}}</label>
                                        ...
                                        <option {{$repetitive_routes->delivery==$deliveries->id?"selected":""}} value="{{$deliveries->id}}">{{"$deliveries->title"}}</option>
                                        @endforeach
                                        </select>
                                        </div>
                                        <div class="col-md-12">
                                        ...
                                        <option {{$repetitive_routes->user==$users->id?"selected":""}} value="{{$users->id}}">{{"($users->mobile)$users->name"}}</option>
                                        @endforeach
                                        </select>
                                        </div>
...
@section('scripts')
    ...
    <script>
        ...
        @php($lastCategory=\App\Models\Category::where("slug",'repetitive_routes')->first())
