...
<i class="sort-icon fa-solid fa-refresh m-1" sortId="{{$wallet_agency->sort}}" style="color: #2e98c5;font-size: 20px;"></i>
{{$counter}}
@elseif($type=="price")
{{$wallet_agency->price}}
@elseif($type=="type")
...
@if(\App\Models\Admin::checkAccess('record','edit','wallet_agency'))
       <li>
            <a class="dropdown-item" href="{{route('record.editform',['slug'=>'wallet_agency','id'=>$wallet_agency->id])}}">
                <i class="bx bx-edit-alt me-1"></i> {{ __('Edit') }}
            </a>
       </li>
@endif
@if(\App\Models\Admin::checkAccess('record','delete','wallet_agency'))
       <li>
            <a class="dropdown-item text-danger" data-bs-toggle="modal" data-bs-target="#basicModal"
            onclick="setModalRoute('{{route('record.destroy',['slug'=>'wallet_agency','id'=>$wallet_agency->id])}}')">
