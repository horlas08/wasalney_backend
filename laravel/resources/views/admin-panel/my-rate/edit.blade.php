...
@section('content')
    ...
    <form action="{{route('record.update',['slug'=>'rate','id'=>$rate->id])}}"
                ...
                <div class="card" id="form-box">
                    ...
                    <div id="sticky-wrapper" class="sticky-wrapper" >
                        <div class="card-header sticky-element bg-label-secondary d-flex justify-content-sm-between align-items-sm-center flex-column flex-sm-row" style="">
                            <h5 class="card-title mb-sm-0 me-2">{{ __('Edit') }} {{__("امتیاز")}}</h5>
                            <div class="action-btns ">
                                <a href="{{route('record.index',['slug'=>'rate'])}}" class="btn btn-outline-primary me-3">
                            ...
                            <div class="row mb-5">
                                <div class="col-lg-11 mx-auto">
                                    <div class="row g-3">
                                        ...
                                        <input id="title" type="text" name="title" value="{{$rate->title}}" class="form-control"  />
                                        </div>
                                        <div class="col-md-12">
                                        <label class="form-label" for="description">{{__("علت")}}</label>
                                        <input id="description" type="text" name="description" value="{{$rate->description}}" class="form-control"  />
                                        </div>
                                        <div class="col-md-12">
                                        <label class="form-label" for="rate">{{__("امتیاز")}}</label>
                                        <input id="rate" type="number" name="rate" value="{{$rate->rate}}" class="form-control"  />
                                        </div>
                                        <div class="col-md-12">
                                        <label class="form-label" for="cost">{{__("ارزش (ریال)")}}</label>
                                        <input id="cost" type="number" name="cost" value="{{$rate->cost}}" class="form-control"  />
                                        </div>
...
@section('scripts')
    ...
    <script>
        ...
        @php($lastCategory=\App\Models\Category::where("slug",'rate')->first())
