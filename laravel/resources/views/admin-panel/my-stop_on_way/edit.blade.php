...
@section('content')
    ...
    <form action="{{route('record.update',['slug'=>'stop_on_way','id'=>$stop_on_way->id])}}"
                ...
                <div class="card" id="form-box">
                    ...
                    <div id="sticky-wrapper" class="sticky-wrapper" >
                        <div class="card-header sticky-element bg-label-secondary d-flex justify-content-sm-between align-items-sm-center flex-column flex-sm-row" style="">
                            <h5 class="card-title mb-sm-0 me-2">{{ __('Edit') }} {{__("اطلاعات توقف در محل")}}</h5>
                            <div class="action-btns ">
                                <a href="{{route('record.index',['slug'=>'stop_on_way','parentSlug'=>$parentSlug,'parentId'=>$parentId])}}" class="btn btn-outline-primary me-3">
                            ...
                            <div class="row mb-5">
                                <div class="col-lg-11 mx-auto">
                                    <div class="row g-3">
                                        ...
                                        <input id="time" type="text" name="time" value="{{$stop_on_way->time}}" class="form-control"  />
                                        </div>
                                        <div class="col-md-12">
                                        <label class="form-label" for="price">{{__("قیمت")}}</label>
                                        <input id="price" type="number" name="price" value="{{$stop_on_way->price}}" class="form-control"  />
                                        </div>
                                        <div class="col-md-12">
                                        <label class="form-label" for="minutes">{{__("زمان بر اساس دقیقه")}}</label>
                                        <input id="minutes" type="number" name="minutes" value="{{$stop_on_way->minutes}}" class="form-control"  />
                                        </div>
