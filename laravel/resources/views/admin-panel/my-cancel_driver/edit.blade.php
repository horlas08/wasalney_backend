...
@section('content')
    ...
    <form action="{{route('record.update',['slug'=>'cancel_driver','id'=>$cancel_driver->id])}}"
                ...
                <div class="card" id="form-box">
                    ...
                    <div id="sticky-wrapper" class="sticky-wrapper" >
                        <div class="card-header sticky-element bg-label-secondary d-flex justify-content-sm-between align-items-sm-center flex-column flex-sm-row" style="">
                            <h5 class="card-title mb-sm-0 me-2">{{ __('Edit') }} {{__("الطلبات الملغية من قبل السائق")}}</h5>
                            <div class="action-btns ">
                                <a href="{{route('record.index',['slug'=>'cancel_driver','parentSlug'=>$parentSlug,'parentId'=>$parentId])}}" class="btn btn-outline-primary me-3">
                            ...
                            <div class="row mb-5">
                                <div class="col-lg-11 mx-auto">
                                    <div class="row g-3">
                                        ...
                                        <option {{$cancel_driver->order==$orders->id?"selected":""}} value="{{$orders->id}}">{{"$orders->price"}}</option>
                                        @endforeach
                                        </select>
                                        </div>
