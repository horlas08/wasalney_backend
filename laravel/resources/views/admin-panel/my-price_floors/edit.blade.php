...
@section('content')
    ...
    <form action="{{route('record.update',['slug'=>'price_floors','id'=>$price_floors->id])}}"
                ...
                <div class="card" id="form-box">
                    ...
                    <div id="sticky-wrapper" class="sticky-wrapper" >
                        <div class="card-header sticky-element bg-label-secondary d-flex justify-content-sm-between align-items-sm-center flex-column flex-sm-row" style="">
                            <h5 class="card-title mb-sm-0 me-2">{{ __('Edit') }} {{__("تحديد سعر الطوابق")}}</h5>
                            <div class="action-btns ">
                                <a href="{{route('record.index',['slug'=>'price_floors','parentSlug'=>$parentSlug,'parentId'=>$parentId])}}" class="btn btn-outline-primary me-3">
                            ...
                            <div class="row mb-5">
                                <div class="col-lg-11 mx-auto">
                                    <div class="row g-3">
                                        ...
                                        <div class="col-md-12">
                                        <label class="form-label" for="elevator_price">{{__("ضریب قیمت حمل بدون آسانسور")}}</label>
                                        <input id="elevator_price" type="number" name="elevator_price" value="{{$price_floors->elevator_price}}" class="form-control"  />
                                        </div>
                                        <div class="col-md-12">
                                        <label class="form-label" for="floor_price">{{__("قیمت هر طبقه")}}</label>
                                        <input id="floor_price" type="number" name="floor_price" value="{{$price_floors->floor_price}}" class="form-control"  />
                                        </div>
                                        <div class="col-md-12">
                                        <label class="form-label" for="name">{{__("نام")}}</label>
                                        <input id="name" type="text" name="name" value="{{$price_floors->name}}" class="form-control"  />
                                        </div>
