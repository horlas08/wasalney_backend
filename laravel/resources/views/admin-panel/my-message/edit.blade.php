...
@section('content')
    ...
    <form action="{{route('record.update',['slug'=>'message','id'=>$message->id])}}"
                ...
                <div class="card" id="form-box">
                    ...
                    <div id="sticky-wrapper" class="sticky-wrapper" >
                        <div class="card-header sticky-element bg-label-secondary d-flex justify-content-sm-between align-items-sm-center flex-column flex-sm-row" style="">
                            <h5 class="card-title mb-sm-0 me-2">{{ __('Edit') }} {{__("پیام ها")}}</h5>
                            <div class="action-btns ">
                                <a href="{{route('record.index',['slug'=>'message','parentSlug'=>$parentSlug,'parentId'=>$parentId])}}" class="btn btn-outline-primary me-3">
                            ...
                            <div class="row mb-5">
                                <div class="col-lg-11 mx-auto">
                                    <div class="row g-3">
                                        ...
                                        <option {{$message->driver==$drivers->id?"selected":""}} value="{{$drivers->id}}">{{"($drivers->mobile) $drivers->name"}}</option>
                                        @endforeach
                                        </select>
                                        </div>
                                        <div class="col-md-12">
                                        ...
                                        <option {{$message->user==$users->id?"selected":""}} value="{{$users->id}}">{{"($users->mobile) $users->name"}}</option>
                                        @endforeach
                                        </select>
                                        </div>
                                        <div class="col-md-12">
                                        <label class="form-label" for="messsage">{{__("پیام")}}</label>
                                        <input id="messsage" type="text" name="messsage" value="{{$message->messsage}}" class="form-control"  />
                                        </div>
                                        <div class="col-md-12">
                                        <input type="hidden" name="user_sent" value="{{ $message->user_sent }}">
                                        </div>
                                        <div class="col-md-12">
                                        <input type="hidden" name="driver_sent" value="{{ $message->driver_sent }}">
                                        </div>
