...
<i class="sort-icon fa-solid fa-refresh m-1" sortId="{{$status->sort}}" style="color: #2e98c5;font-size: 20px;"></i>
{{$counter}}
@elseif($type=="status")
...
@if(\App\Models\Admin::checkAccess('record','edit','status'))
       <li>
            <a class="dropdown-item" href="{{route('record.editform',['slug'=>'status','id'=>$status->id])}}">
                <i class="bx bx-edit-alt me-1"></i> {{ __('Edit') }}
            </a>
       </li>
@endif
@if(\App\Models\Admin::checkAccess('record','delete','status'))
       <li>
            <a class="dropdown-item text-danger" data-bs-toggle="modal" data-bs-target="#basicModal"
            onclick="setModalRoute('{{route('record.destroy',['slug'=>'status','id'=>$status->id])}}')">
