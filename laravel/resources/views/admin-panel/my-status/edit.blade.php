...
@section('content')
    ...
    <form action="{{route('record.update',['slug'=>'status','id'=>$status->id])}}"
                ...
                <div class="card" id="form-box">
                    ...
                    <div id="sticky-wrapper" class="sticky-wrapper" >
                        <div class="card-header sticky-element bg-label-secondary d-flex justify-content-sm-between align-items-sm-center flex-column flex-sm-row" style="">
                            <h5 class="card-title mb-sm-0 me-2">{{ __('Edit') }} {{__("وضعیت")}}</h5>
                            <div class="action-btns ">
                                <a href="{{route('record.index',['slug'=>'status','parentSlug'=>$parentSlug,'parentId'=>$parentId])}}" class="btn btn-outline-primary me-3">
                            ...
                            <div class="row mb-5">
                                <div class="col-lg-11 mx-auto">
                                    <div class="row g-3">
                                        ...
                                        <div class="col-md-12">
                                        <label class="form-label" for="status">{{__("وضعیت")}}</label>
                                        <select id="status" name="status" class="select2 form-select  form-select-lg"  data-width="100%">
                                        <option value=" ">{{__('not selected')}}</option>
                                        @foreach(\App\Models\MyStatuses::orderBy('sort','DESC')->get() as $statuses)
                                        <option {{$status->status==$statuses->id?"selected":""}} value="{{$statuses->id}}">{{"$statuses->title"}}</option>
                                        @endforeach
                                        </select>
                                        </div>
