...
@section('content')
    ...
    <form action="{{route('record.update',['slug'=>'floor_detail','id'=>$floor_detail->id])}}"
                ...
                <div class="card" id="form-box">
                    ...
                    <div id="sticky-wrapper" class="sticky-wrapper" >
                        <div class="card-header sticky-element bg-label-secondary d-flex justify-content-sm-between align-items-sm-center flex-column flex-sm-row" style="">
                            <h5 class="card-title mb-sm-0 me-2">{{ __('Edit') }} {{__("جزئیات طبقات")}}</h5>
                            <div class="action-btns ">
                                <a href="{{route('record.index',['slug'=>'floor_detail'])}}" class="btn btn-outline-primary me-3">
                            ...
                            <div class="row mb-5">
                                <div class="col-lg-11 mx-auto">
                                    <div class="row g-3">
                                        ...
                                        <div class="col-md-12">
                                        <label class="form-label" for="count">{{__("تعداد طبقات")}}</label>
                                        <input id="count" type="number" name="count" value="{{$floor_detail->count}}" class="form-control"  />
                                        </div>
                                        <div class="col-md-12">
                                        <label class="form-label" for="price">{{__("قیمت هر طبقه")}}</label>
                                        <input id="price" type="number" name="price" value="{{$floor_detail->price}}" class="form-control"  />
                                        </div>
                                        <div class="col-md-12">
                                        <input type="hidden" name="elevator" value="{{ $floor_detail->elevator }}">
                                        </div>
                                        <div class="col-md-12">
                                        <label class="form-label" for="place">{{__("محل")}}</label>
                                        <input id="place" type="text" name="place" value="{{$floor_detail->place}}" class="form-control"  />
                                        </div>
...
@section('scripts')
    ...
    <script>
        ...
        @php($lastCategory=\App\Models\Category::where("slug",'floor_detail')->first())
