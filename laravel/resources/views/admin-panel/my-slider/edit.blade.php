...
@section('content')
    ...
    <form action="{{route('record.update',['slug'=>'slider','id'=>$slider->id])}}"
                ...
                <div class="card" id="form-box">
                    ...
                    <div id="sticky-wrapper" class="sticky-wrapper" >
                        <div class="card-header sticky-element bg-label-secondary d-flex justify-content-sm-between align-items-sm-center flex-column flex-sm-row" style="">
                            <h5 class="card-title mb-sm-0 me-2">{{ __('Edit') }} {{__("اسلایدر")}}</h5>
                            <div class="action-btns ">
                                <a href="{{route('record.index',['slug'=>'slider'])}}" class="btn btn-outline-primary me-3">
                            ...
                            <div class="row mb-5">
                                <div class="col-lg-11 mx-auto">
                                    <div class="row g-3">
                                        ...
                                        <div style="display: flex" deleteUrl="{{route('file.delete',['slug'=>'slider','fieldName'=>'image','id'=>$slider->id])}}">
                                        @if($slider->image!=null)                                        <div style="position: relative">
                                        <a href="{{"/files/".$slider->image}}" target="_blank">
                                        <img src="{{"/files/".$slider->image}}" height="60" width="60" onerror="this.src='/admin-panel/images/file.png'">
                                        </a>
                                        <i id="btnDelete-image" onclick="deleteFile(this,'image')" fileName="{{last(explode('/',$slider->image))}}" class="fa-solid fa-trash" style="position: absolute;top: 5px;right: 5%;color: red;cursor: pointer"></i>
                                        <i data-text="{{"/files/".$slider->image}}" class="fa-solid fa-copy copy" style="position: absolute;top: 5px;left: 5%;color: orangered;cursor: pointer"></i>
                                        </div>
                                        @endif
                                        </div>
                                        </div>
                                        <div class="col-md-12">
                                        <label class="form-label" for="link">{{__("لینک")}}</label>
                                        <input id="link" type="url" name="link" value="{{$slider->link}}" class="form-control"  style="direction:ltr !important;"/>
                                        </div>
...
@section('scripts')
    ...
    <script>
        ...
        @php($lastCategory=\App\Models\Category::where("slug",'slider')->first())
