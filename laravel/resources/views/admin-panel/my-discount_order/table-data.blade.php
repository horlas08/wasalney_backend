@if($type=="datatable-counter")
<i class="sort-icon fa-solid fa-refresh m-1" sortId="{{$discount_order->sort}}" style="color: #2e98c5;font-size: 20px;"></i>
{{$counter}}
@elseif($type=="user")
@if($users=\App\Models\MyUsers::find($discount_order->user))
{{"($users->mobile) $users->name"}}
@endif
@elseif($type=="order")
@if($orders=\App\Models\MyOrders::find($discount_order->order))
{{"$orders->code"}}
@endif
@elseif($type=="create_at")
@if(getLang()=='fa')
{{\Morilog\Jalali\Jalalian::fromDateTime($discount_order->created_at)->format('%A, %d %B %Y H:i')}}
@else
{{\Illuminate\Support\Carbon::createFromFormat('Y-m-d H:i:s', $discount_order->created_at)->format('M d Y H:i')}}
@endif
@else
<div class="btn-group">
<button type="button" class="btn btn-outline-primary dropdown-toggle" data-bs-toggle="dropdown" aria-expanded="false">{{ __('Actions') }}</button>
<ul class="dropdown-menu" style="">
@if(\App\Models\Admin::checkAccess('record','edit','discount_order'))
       <li>
            <a class="dropdown-item" href="{{route('record.editform',['slug'=>'discount_order','id'=>$discount_order->id])}}">
                <i class="bx bx-edit-alt me-1"></i> {{ __('Edit') }}
            </a>
       </li>
@endif
@if(\App\Models\Admin::checkAccess('record','delete','discount_order'))
       <li>
            <a class="dropdown-item text-danger" data-bs-toggle="modal" data-bs-target="#basicModal"
            onclick="setModalRoute('{{route('record.destroy',['slug'=>'discount_order','id'=>$discount_order->id])}}')">
                <i class="bx bx-trash me-1"></i>{{ __('Delete') }}
            </a>
       </li>
@endif
</ul>
</div>
@endif
