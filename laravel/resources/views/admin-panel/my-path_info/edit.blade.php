...
@section('content')
    ...
    <form action="{{route('record.update',['slug'=>'path_info','id'=>$path_info->id])}}"
                ...
                <div class="card" id="form-box">
                    ...
                    <div id="sticky-wrapper" class="sticky-wrapper" >
                        <div class="card-header sticky-element bg-label-secondary d-flex justify-content-sm-between align-items-sm-center flex-column flex-sm-row" style="">
                            <h5 class="card-title mb-sm-0 me-2">{{ __('Edit') }} {{__("اطلاعات مسیر")}}</h5>
                            <div class="action-btns ">
                                <a href="{{route('record.index',['slug'=>'path_info','parentSlug'=>$parentSlug,'parentId'=>$parentId])}}" class="btn btn-outline-primary me-3">
                            ...
                            <div class="row mb-5">
                                <div class="col-lg-11 mx-auto">
                                    <div class="row g-3">
                                        ...
                                        <input id="name" type="text" name="name" value="{{$path_info->name}}" class="form-control"  />
                                        </div>
                                        <div class="col-md-12">
                                        <label class="form-label" for="lat">{{__("lat")}}</label>
                                        <input id="lat" type="text" name="lat" value="{{$path_info->lat}}" class="form-control"  />
                                        </div>
                                        <div class="col-md-12">
                                        <label class="form-label" for="long">{{__("long")}}</label>
                                        <input id="long" type="text" name="long" value="{{$path_info->long}}" class="form-control"  />
                                        </div>
                                        <div class="col-md-12">
                                        <label class="form-label" for="address">{{__("آدرس")}}</label>
                                        <input id="address" type="text" name="address" value="{{$path_info->address}}" class="form-control"  />
                                        </div>
                                        <div class="col-md-12">
                                        <label class="form-label" for="description">{{__("توضیحات")}}</label>
                                        <input id="description" type="text" name="description" value="{{$path_info->description}}" class="form-control"  />
                                        </div>
                                        <div class="col-md-12">
                                        <label class="form-label" for="phone">{{__("شماره تلفن")}}</label>
                                        <input id="phone" type="text" name="phone" value="{{$path_info->phone}}" class="form-control"  />
                                        </div>
                                        <div class="col-md-12">
                                        <label class="form-label" for="house_number">{{__("پلاک")}}</label>
                                        <input id="house_number" type="text" name="house_number" value="{{$path_info->house_number}}" class="form-control"  />
                                        </div>
                                        <div class="col-md-12">
                                        <label class="form-label" for="unit">{{__("واحد")}}</label>
                                        <input id="unit" type="text" name="unit" value="{{$path_info->unit}}" class="form-control"  />
                                        </div>
                                        <div class="col-md-12">
                                        <label class="form-label" for="type">{{__("نوع")}}</label>
                                        <input id="type" type="text" name="type" value="{{$path_info->type}}" class="form-control"  />
                                        </div>
...
@section('scripts')
    ...
    <script>
        ...
        @endwhile
        @php($lastCategory=isset($lastCategory)?$lastCategory:(\App\Models\Category::where("slug",'path_info')->first()))
