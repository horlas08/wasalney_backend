...
@section('content')
    ...
    <form action="{{route('record.update',['slug'=>'wallet','id'=>$wallet->id])}}"
                ...
                <div class="card" id="form-box">
                    ...
                    <div id="sticky-wrapper" class="sticky-wrapper" >
                        <div class="card-header sticky-element bg-label-secondary d-flex justify-content-sm-between align-items-sm-center flex-column flex-sm-row" style="">
                            <h5 class="card-title mb-sm-0 me-2">{{ __('Edit') }} {{__("کیف پول")}}</h5>
                            <div class="action-btns ">
                                <a href="{{route('record.index',['slug'=>'wallet','parentSlug'=>$parentSlug,'parentId'=>$parentId])}}" class="btn btn-outline-primary me-3">
                            ...
                            <div class="row mb-5">
                                <div class="col-lg-11 mx-auto">
                                    <div class="row g-3">
                                        ...
                                        <option {{$wallet->type==$transaction_types->id?"selected":""}} value="{{$transaction_types->id}}">{{"$transaction_types->title"}}</option>
                                        @endforeach
                                        </select>
                                        </div>
                                        <div class="col-md-12">
                                        <label class="form-label" for="price">{{__("قیمت")}}</label>
                                        <input id="price" type="number" name="price" value="{{$wallet->price}}" class="form-control"  />
                                        </div>
                                        <div class="col-md-12">
                                        <label class="form-label" for="net_price">{{__("خالص دریافتی")}}</label>
                                        <input id="net_price" type="number" name="net_price" value="{{$wallet->net_price}}" class="form-control"  />
                                        </div>
                                        <div class="col-md-12">
                                        <label class="form-label" for="description">{{__("توضیحات")}}</label>
                                        <input id="description" type="text" name="description" value="{{$wallet->description}}" class="form-control"  />
                                        </div>
                                        <div class="col-md-12">
                                        <label class="form-label" for="title">{{__("عنوان")}}</label>
                                        <input id="title" type="text" name="title" value="{{$wallet->title}}" class="form-control"  />
                                        </div>
                                        <div class="col-md-12">
                                        <label class="form-label" for="order">{{__("سفارش")}}</label>
                                        <select id="order" name="order" class="select2 form-select  form-select-lg"  data-width="100%">
                                        <option value=" ">{{__('not selected')}}</option>
                                        @foreach(\App\Models\MyOrders::orderBy('sort','DESC')->get() as $orders)
                                        <option {{$wallet->order==$orders->id?"selected":""}} value="{{$orders->id}}">{{"$orders->price"}}</option>
                                        @endforeach
                                        </select>
                                        </div>
...
@section('scripts')
    ...
    <script>
        ...
        @endwhile
        @php($lastCategory=isset($lastCategory)?$lastCategory:(\App\Models\Category::where("slug",'wallet')->first()))
