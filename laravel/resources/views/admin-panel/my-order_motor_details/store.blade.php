...
@section('content')
    ...
    <form action="{{route('record.store',['slug'=>'order_motor_details'])}}"
                ...
                <div class="card" >
                    ...
                    <div id="sticky-wrapper" class="sticky-wrapper" >
                        <div class="card-header sticky-element bg-label-secondary d-flex justify-content-sm-between align-items-sm-center flex-column flex-sm-row" style="">
                            <h5 class="card-title mb-sm-0 me-2">{{ __('Store') }} {{__("جزئیات سفارشات موتور باکس")}}</h5>
                            <div class="action-btns">
                                <a href="{{route('record.index',['slug'=>'order_motor_details'])}}" class="btn btn-outline-primary me-3">
                            ...
                            <div class="row mb-5">
                                <div class="col-lg-11 mx-auto">
                                    <div class="row g-3">
                                        ...
                                        <option  value="{{$orders->id}}">{{"$orders->price"}}</option>
                                        @endforeach
                                        </select>
                                        </div>
                                        <div class="col-md-12">
                                        <label class="form-label" for="type_parcel">{{__("نوع مرسوله")}}</label>
                                        <select id="type_parcel" name="type_parcel" class="select2 form-select  form-select-lg"  data-width="100%">
                                        <option value=" ">{{__('not selected')}}</option>
                                        @foreach(\App\Models\MyType_Parcel::orderBy('sort','DESC')->get() as $type_parcel)
                                        <option  value="{{$type_parcel->id}}">{{"$type_parcel->title"}}</option>
                                        @endforeach
                                        </select>
                                        </div>
                                        <div class="col-md-12">
                                        <label class="form-label" for="price_parcel">{{__("ارزش مرسوله")}}</label>
                                        <select id="price_parcel" name="price_parcel" class="select2 form-select  form-select-lg"  data-width="100%">
                                        <option value=" ">{{__('not selected')}}</option>
                                        @foreach(\App\Models\MyPrice_Parcel::orderBy('sort','DESC')->get() as $price_parcel)
                                        <option  value="{{$price_parcel->id}}">{{"$price_parcel->title"}}</option>
                                        @endforeach
                                        </select>
                                        </div>
