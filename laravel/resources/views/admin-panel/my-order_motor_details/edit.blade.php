...
@section('content')
    ...
    <form action="{{route('record.update',['slug'=>'order_motor_details','id'=>$order_motor_details->id])}}"
                ...
                <div class="card" id="form-box">
                    ...
                    <div id="sticky-wrapper" class="sticky-wrapper" >
                        <div class="card-header sticky-element bg-label-secondary d-flex justify-content-sm-between align-items-sm-center flex-column flex-sm-row" style="">
                            <h5 class="card-title mb-sm-0 me-2">{{ __('Edit') }} {{__("جزئیات سفارشات موتور باکس")}}</h5>
                            ...
                            <div class="row mb-5">
                                <div class="col-lg-11 mx-auto">
                                    <div class="row g-3">
                                        ...
                                        <option {{$order_motor_details->order_id==$orders->id?"selected":""}} value="{{$orders->id}}">{{"$orders->price"}}</option>
                                        @endforeach
                                        </select>
                                        </div>
                                        <div class="col-md-12">
                                        ...
                                        <option {{$order_motor_details->type_parcel==$type_parcel->id?"selected":""}} value="{{$type_parcel->id}}">{{"$type_parcel->title"}}</option>
                                        @endforeach
                                        </select>
                                        </div>
                                        <div class="col-md-12">
                                        ...
                                        <option {{$order_motor_details->price_parcel==$price_parcel->id?"selected":""}} value="{{$price_parcel->id}}">{{"$price_parcel->title"}}</option>
                                        @endforeach
                                        </select>
                                        </div>
