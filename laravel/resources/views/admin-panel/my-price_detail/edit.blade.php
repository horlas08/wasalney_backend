...
@section('content')
    ...
    <form action="{{route('record.update',['slug'=>'price_detail','id'=>$price_detail->id])}}"
                ...
                <div class="card" id="form-box">
                    ...
                    <div id="sticky-wrapper" class="sticky-wrapper" >
                        <div class="card-header sticky-element bg-label-secondary d-flex justify-content-sm-between align-items-sm-center flex-column flex-sm-row" style="">
                            <h5 class="card-title mb-sm-0 me-2">{{ __('Edit') }} {{__("جزئیات قیمت")}}</h5>
                            <div class="action-btns ">
                                <a href="{{route('record.index',['slug'=>'price_detail','parentSlug'=>$parentSlug,'parentId'=>$parentId])}}" class="btn btn-outline-primary me-3">
                            ...
                            <div class="row mb-5">
                                <div class="col-lg-11 mx-auto">
                                    <div class="row g-3">
                                        ...
                                        <div class="col-md-12">
                                        <label class="form-label" for="base_price">{{__("قیمت پایه سرویس")}}</label>
                                        <input id="base_price" type="number" name="base_price" value="{{$price_detail->base_price}}" class="form-control"  />
                                        </div>
                                        <div class="col-md-12">
                                        <label class="form-label" for="multiply_price">{{__("ضریب قیمت سرویس")}}</label>
                                        <input id="multiply_price" type="number" name="multiply_price" value="{{$price_detail->multiply_price}}" class="form-control"  />
                                        </div>
                                        <div class="col-md-12">
                                        <label class="form-label" for="hurry_percent">{{__("درصد قیمت عجله دارم")}}</label>
                                        <input id="hurry_percent" type="number" name="hurry_percent" value="{{$price_detail->hurry_percent}}" class="form-control"  />
                                        </div>
                                        <div class="col-md-12">
                                        <label class="form-label" for="back_price">{{__("قیمت بازگشت")}}</label>
                                        <input id="back_price" type="number" name="back_price" value="{{$price_detail->back_price}}" class="form-control"  />
                                        </div>
                                        <div class="col-md-12">
                                        <label class="form-label" for="decrease_percent">{{__("درصد کاهش قیمت سرویس اقتصادی")}}</label>
                                        <input id="decrease_percent" type="number" name="decrease_percent" value="{{$price_detail->decrease_percent}}" class="form-control"  />
                                        </div>
                                        <div class="col-md-12">
                                        <label class="form-label" for="final_price">{{__("قیمت نهایی")}}</label>
                                        <input id="final_price" type="number" name="final_price" value="{{$price_detail->final_price}}" class="form-control"  />
                                        </div>
...
@section('scripts')
    ...
    <script>
        ...
        @endwhile
        @php($lastCategory=isset($lastCategory)?$lastCategory:(\App\Models\Category::where("slug",'price_detail')->first()))
